...
def create_twitter_url(data, max_results=100):
...
def mean_score(sentiments):
    # Load the Sentiments from the JSON Payload
    json_data = json.loads(sentiments)
    # Get the sentiment scores from the documents
    scores = [doc['score'] for doc in json_data['documents']]
...
def main():
    data = process_yaml()
    url = create_twitter_url(data)
    bearer_token = create_bearer_token(data)
    json = twitter_auth_and_connect(bearer_token, url)
    documents = lang_data_shape(json)
